"""
Página Dimensão Urbana
Indicadores urbanos com filtros e visualização hierárquica
"""
...
st.set_page_config(
    page_title="Dimensão Urbana - RAPI",
    page_icon="🏙️",
    layout="wide"
)
...
# Header
render_header("🏙️ Dimensão Urbana", "Indicadores de Desenvolvimento Urbano e Infraestrutura")
...
try:
    ...
    total_original = df_urb[df_urb['ano'] == selected_year]['id'].nunique()
    ...
    # Tabela hierárquica
    st.markdown('<div class="section-card">', unsafe_allow_html=True)
    st.subheader(f"📋 Indicadores Urbanos - {selected_year}")
    ...
    st.markdown("---")
    st.info("""
    💡 **Sobre os Indicadores Urbanos:**
    
    Esta dimensão abrange indicadores relacionados a:
    - 🏘️ Habitação e moradia
    - 🚇 Mobilidade urbana e transporte
    - 🏗️ Infraestrutura e planejamento urbano
    - 🚦 Segurança viária
    - 🏞️ Espaços públicos e áreas verdes
